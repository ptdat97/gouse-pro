...
export interface components {
    schemas: {
        ...
        SKUSummary: {
             ...
             *
             *     **Chỉ là KÝ HIỆU.** Hệ quy chiếu làm ký hiệu này có nghĩa nằm ở
             *     `ProductDetail.size_chart.system` (ALPHA, EU, US...) cùng số đo
             *     thực tế — vì hệ size gắn với (thương hiệu, loại hàng), không gắn
             *     với từng SKU.
             *
        ...
        Variant: {
             ...
             *     có thuộc tính khác nhau (áo có độ dài tay, quần có kiểu ống).
             *
             *     Nhóm màu để LỌC đã có: `color_family` suy ra từ tên màu và được
        ...
        SellerOffer: {
            ...
            status: "DRAFT" | "ACTIVE" | "SUSPENDED" | "ARCHIVED";
            /**
             * @description Khách mua được hay không — **cùng một câu hỏi và cùng một câu trả
             *     lời với `Offer.is_sellable` của trang công khai**. Người bán hỏi
             *     "hàng của tôi bán được không" thì phải nhận đúng câu khách nhận
             *     được, không phải một phiên bản dễ dãi hơn.
             *
             *     **Đừng suy ra từ `status` ở giao diện**: quy tắc còn phụ thuộc tồn
             *     kho VÀ việc seller có bị đình chỉ không, và suy ở hai nơi thì hai
             *     nơi sẽ lệch. Chính máy chủ đã từng vi phạm đúng câu này — trường
             *     được tính bằng `status == ACTIVE`, nên Seller Center không bao giờ
             *     có tín hiệu hết hàng.
             *
             *     **`status: ACTIVE` kèm `is_sellable: false` là tín hiệu HẾT HÀNG**
             *     (hoặc tài khoản nhà bán đang bị đình chỉ). Đó là cặp giá trị giao
             *     diện nhà bán phải đọc: hết hàng KHÔNG phải một trạng thái của offer
             *     (xem `Offer.status`), nên `status` một mình không nói lên điều đó.
             */
            is_sellable: boolean;
            created_at: components["schemas"]["Timestamp"];
        };
    ...
    listProducts: {
        parameters: {
            query?: {
                ...
                /**
                 * @description **CHƯA CÀI ĐẶT — máy chủ BỎ QUA tham số này.**
                 *
                 *     Giá nằm ở module `marketplace` (offer), tồn kho ở `inventory`.
                 *     Lọc theo chúng cần JOIN xuyên module — thứ kiến trúc cấm, và cấm
                 *     có lý do: module nào sở hữu bảng nào là ranh giới giữ cho chúng
                 *     thay đổi độc lập.
                 *
                 *     Lọc ở tầng ứng dụng KHÔNG lách được: lấy 50 sản phẩm rồi tự loại
                 *     những cái ngoài khoảng giá sẽ trả về ít hơn 50, và trang 2 bỏ
                 *     sót. Phân trang chỉ đúng khi việc lọc nằm TRONG truy vấn.
                 *
                 *     Hướng đi là một read model `product_search` do event cập nhật —
                 *     xem backlog PH-39. Ghi ở đây vì một tham số khai mà không cài là
                 *     một bộ lọc im lặng không làm gì: client gửi lên, nhận về danh
                 *     sách ĐỦ, và không có lỗi nào để lần theo.
                 */
                price_min?: number;
                /** @description Xem `price_min` — **CHƯA CÀI ĐẶT**, máy chủ bỏ qua. */
                price_max?: number;
                /** @description Xem `price_min` — **CHƯA CÀI ĐẶT**, máy chủ bỏ qua. */
                in_stock?: boolean;
                gender_target?: "MEN" | "WOMEN" | "UNISEX" | "KIDS";
        ...
        responses: {
            ...
            200: {
                ...
                content: {
                     ...
                     *           "reason": "cần dữ liệu của module returns; đó là module ở tầng khác nên phải qua một read model chung, chưa dựng"
                     *         },
                     *         {
                     *           "name": "average_rating",
                     *           "reason": "hệ thống chưa có đánh giá của khách"
                     *         },
                     *         {
                     *           "name": "inventory_accuracy",
                     *           "reason": "chưa thống nhất công thức tính lệch kiểm kê"
                     *         },
                     *         {
                     *           "name": "buy_box_win_rate",
                     *           "reason": "buy box tính tại thời điểm hỏi và không được lưu lại, nên không có lịch sử để tính tỷ lệ"
                     *         }
                     *       ],
                     *       "impact": {
                     *         "message": "Giao đúng hạn đang ở 94%, dưới ngưỡng 95%. Hạn bàn giao là 48 giờ kể từ khi đơn được tạo."
                     *       }
                     *     }
                     */
                    "application/json": {
                        ...
                        /**
                         * @description Số đơn thực hiện trong kỳ, tức MẪU dùng để tính mọi tỷ
                         *     lệ ở trên.
                         *
                         *     Bắt buộc có: "tỷ lệ hủy 5%" mà không nói của bao nhiêu
                         *     đơn là một khẳng định không kiểm chứng được, và đó
                         *     chính là hộp đen mà endpoint này sinh ra để tránh.
                         *
                         *     Mẫu dưới ngưỡng tối thiểu thì `metrics` TRỐNG — chấm
                         *     một gian hàng mới mở là NGHIÊM TRỌNG vì họ hủy đúng
                         *     một đơn là bất công.
                         */
                        sample_size?: number;
                        /**
                         * @description Thước đo dùng để quyết định một đơn có "đúng hạn" hay
                         *     không, tính từ lúc đơn thực hiện được tạo.
                         *
                         *     Trả ra để nhà bán luôn đọc được tiêu chí đang dùng để
                         *     chấm mình.
                         */
                        shipping_sla_hours?: number;
                        /**
                         * @description MỌI chỉ số không có trong `metrics`, kèm lý do. Hai
                         *     loại lý do, và chúng khác nhau về bản chất:
                         *
                         *     - **thiếu nguồn dữ liệu** — vĩnh viễn cho tới khi có
                         *       người xây (`average_rating`, `buy_box_win_rate`…)
                         *     - **chưa đủ mẫu** — đo được, nhưng kỳ này quá ít đơn
                         *       để con số có nghĩa. Tự hết khi gian hàng bán thêm.
                         *       Lý do nêu rõ cần bao nhiêu và hiện có bao nhiêu.
                         *
                         *     Loại thứ hai thêm 17/09/2026. Trước đó một chỉ số đo
                         *     được mà thiếu mẫu chỉ đơn giản BIẾN MẤT khỏi `metrics`
                         *     không một lời — nhà bán không có cách nào biết vì sao,
                         *     tức đúng thứ hộp đen endpoint này sinh ra để tránh, chỉ
                         *     khác là ở phía người viết API.
                         *
                         *     Bất biến: mỗi chỉ số đo được nằm ở ĐÚNG MỘT trong hai
                         *     danh sách, không bao giờ cả hai và không bao giờ không
                         *     danh sách nào (`TestKhongChiSoNaoVuaDoVuaChuaDo`).
                         */
                        not_measured?: {
                            name?: string;
                            reason?: string;
                        ...
                        impact?: {
                            /**
                             * @description Việc NÊN LÀM tiếp theo, không chỉ tình trạng hiện
                             *     tại. Nêu đúng MỘT chỉ số tệ nhất: một danh sách
                             *     việc cần làm dài thì không ai biết bắt đầu từ đâu.
                             */
                            message?: string;
                        };
                    };
    ...
    getCustomerAsAdmin: {
        parameters: {
            ...
            header: {
                 ...
                 *
                 *     Tối thiểu 20 ký tự, và không được là chuỗi rác lặp lại (`test`,
                 *     `fix`, `abc`, …) — cùng ngưỡng với mọi thao tác nhạy cảm khác.
                 *     Một lý do trống hoặc vô nghĩa làm toàn bộ nhật ký truy cập trở
                 *     nên vô dụng: nó ghi lại rằng có người đã xem, nhưng không trả lời
